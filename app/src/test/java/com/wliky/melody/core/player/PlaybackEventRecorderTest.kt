...
class PlaybackEventRecorderTest {
    ...
    fun `累计播放时长达到阈值才上报`() {
        ...
        repeat(3) { index -> recorder.onPosition(positionMs = index * 1_000L, playing = true) }
        recorder.onSongChanged(null, positionMs = 0)

        assertEquals(1, events.size)
        assertEquals("1", events[0].songId)
        assertEquals(2L, events[0].durationSeconds)
    }
    ...
    fun `拖动进度条造成的大跨度前进不计入时长`() {
        recorder.onSongChanged(song("1"), positionMs = 0)
        recorder.onPosition(positionMs = 2_000L, playing = true)   // +2s 正常播放
        recorder.onPosition(positionMs = 90_000L, playing = true)  // 大跨度：视为拖动
        recorder.onPosition(positionMs = 91_000L, playing = true)  // +1s
        recorder.flush()

        assertEquals(1, events.size)
        assertEquals(3L, events[0].durationSeconds)
    }
    ...
    fun `切歌会结算上一首并开启新事件`() {
        ...
        recorder.onPosition(positionMs = 8_000L, playing = true)
        recorder.onSongChanged(song("B"), positionMs = 0)
        recorder.onPosition(positionMs = 6_000L, playing = true)
        recorder.flush()

        assertEquals(2, events.size)
        assertEquals("A", events[0].songId)
        assertEquals(8L, events[0].durationSeconds)
        assertEquals("B", events[1].songId)
        assertEquals(6L, events[1].durationSeconds)
        // eventId 由 UUID 生成，保证幂等去重
    ...
    fun `完成标记会带到事件上`() {
        recorder.onSongChanged(song("A"), positionMs = 0)
        recorder.onPosition(positionMs = 10_000L, playing = true)
