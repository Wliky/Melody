...
| 变更 | 说明 |
| --- | --- |
| **数据源固定为自建 API 服务** | 统一走作者部署的 `https://music.api.005201.xyz`（[api-enhanced](https://github.com/Wliky/api-enhanced)），纯 HTTP、登录最稳。**已移除数据源切换、直连模式与演示模式的运行时入口** |
| **登录修复（验证码 400 / 扫码无响应）** | 根因是 api-enhanced 把网易返回的业务码映射成 HTTP 状态码（验证码错误 code=503 → HTTP 503）。现在客户端对登录接口先解析 body 里的 `code`/`msg` 翻译成友好文案，而不是笼统报「HTTP 400」 |
| **登录才能播放** | 播放地址解析层加了登录门禁：未登录（没有 `MUSIC_U`）时无法解析任何音源，登录页是唯一入口。首页未登录会显示明确的登录引导横幅 |
| **登录三条通路** | 默认**手机验证码**（国内最稳、不碰密码）；**扫码**；**Cookie** 兜底。三者一键互切 |
| **同步默认开启、无手动入口** | 听歌记录**听歌即自动记录**：入队 → 防抖合并提交 → 断网补交 → 定时兜底重试。界面上**没有**任何「立即同步」按钮，只在设置里留一个开关 |
| **设置页重构** | 参考 NeriPlayer 的分组式设置：账号（登录/退出置顶）→ 播放音质 → 外观 → 播放与记录 → 关于 |
...
- **听歌记录默认自动同步**：播放行为自动记录并提交到**当前使用的自建 API 服务**（`https://music.api.005201.xyz`），
  该服务只做接口转发、不保存登录凭据。你随时可以在「设置 → 播放与记录」里关掉上传，本地历史不受影响。
...
| **登录才能播放**：未登录时音源解析被拦截，登录页是唯一入口 | ✅ |
| 单元测试（61 个用例，歌词、加解密、事件队列、Cookie、风控码、DTO 兼容性、分页） | ✅ |

数据源固定为**自建 API 服务**（不再暴露切换）：

| 说明 |
| --- |
| 统一指向 `https://music.api.005201.xyz`（[api-enhanced](https://github.com/Wliky/api-enhanced)），纯 HTTP、兼容性最好、登录最稳。登录后才能使用播放、歌单、收藏与听歌记录同步 |
...
├── data/
│   ├── netease/       数据源实现 + DTO + 映射（运行期固定走自建 API 服务）
...
- **登录门禁**：音源地址解析层在未登录（会话无 `MUSIC_U`）时直接拒绝解析，
  播放功能在没有登录态时不可用，登录页是唯一入口。
- **登录三条通路 + Cookie 兜底**：手机验证码（默认）→ 扫码 → Cookie，三者一键互切；
  服务端回传的业务码会先解析成友好文案再展示，而不是笼统的 HTTP 状态码。
...
- 数据源固定为自建 API 服务（`https://music.api.005201.xyz`），移除数据源切换 / 直连 / 演示模式的运行时入口。
- 修复验证码登录 400：api-enhanced 把业务码映射成 HTTP 状态码，客户端现先解析 body 的 `code`/`msg` 翻译成友好文案。
- 修复扫码登录：登录成功后的 Cookie 正确清洗落盘，登录态唯一判据改为 `MUSIC_U`。
- 登录才能播放：音源解析层加登录门禁，未登录无法播放，首页显示登录引导横幅。
- 登录三条通路：手机验证码（默认）/ 扫码 / Cookie，一键互切。
- 同步默认开启且**无手动入口**：历史页不再展示同步状态，只在设置留「同步听歌记录」开关。
- 设置页重构：参考 NeriPlayer 分组（账号置顶 → 音质 → 外观 → 播放与记录 → 关于）。
